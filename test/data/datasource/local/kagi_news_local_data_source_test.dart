...
void main() {
  ...
  group('SharedPreferences operations', () {
    test(
      'getLastSavedTimestamp returns null when no timestamp is saved',
      () async {
    ...
    test('saveCategories and getCategories work correctly', () async {
      ...
      expect(retrievedCategories?.categories.first.name, 'Tech');
      expect(retrievedCategories?.categories.first.file, 'tech.json');
    });
  });

  group('CategoryDetailsDao operations', () {
    test(
      'getCategoryDetails returns null when no category details are found',
      () async {
    ...
    test('getCategoryDetails correctly deserializes onThisDay data', () async {
      ...
      expect(details?.onThisDayItems?.first.year, '1969');
    ...
    test(
      'saveCategoryDetails correctly calls dao for onThisDay data',
      () async {
        final onThisDayResponse = NewsCategoryDetailsResponse(
          category: null,
          timestamp: 1644048227,
    ...
    test(
      ...
      () async {
        ...
        expect(allDetails.first.category, 'Tech');
    ...
    test('getCategoryDetailsByName handles nullable category', () async {
      ...
      final onThisDayDetails = await dataSource.getCategoryDetailsByName(
        'onthisday',
      );
